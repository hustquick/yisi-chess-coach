...
const enginePath = join(root, ".local", "stockfish", "stockfish");
const logicalCores = availableParallelism();
const requestedThreads = Number(process.env.STOCKFISH_THREADS);
...
const port = Number(process.env.STOCKFISH_PORT ?? 8788);
const engine = spawn(enginePath, [], { cwd: join(root, ".local", "stockfish"), stdio: ["pipe", "pipe", "inherit"] });
...
createServer((req, res) => {
  ...
  if (req.url === "/health") { res.writeHead(200, { ...cors, "Content-Type": "application/json" }); res.end(JSON.stringify({ engine: "Stockfish", mode: "native", threads: threadCount, network: "local" })); return; }
