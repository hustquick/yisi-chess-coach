...
const worker = {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/api/stockfish-network") {
      const upstream = await fetch(
        "https://github.com/official-stockfish/Networks/releases/download/master-net/stockfish.nnue",
        { cf: { cacheEverything: true, cacheTtl: 86400 } } as RequestInit,
      );
      if (!upstream.ok) return new Response("Stockfish network unavailable", { status: 502 });
