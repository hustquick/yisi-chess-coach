...
cpp_files = Dir.glob(File.join(ROOT, "ThirdParty", "Stockfish", "src", "**", "*.cpp")).sort
  .reject { |path| path.end_with?("/main.cpp") || path.include?("/universal/") }
  .map { |path| path.delete_prefix("#{ROOT}/") }
source_files = swift_files + ["Bridge/StockfishBridge.mm"] + cpp_files
resource_files = ["App/Assets.xcassets", "App/Resources/AppIcon.png", "App/Resources/AppIconDark.png", "App/Resources/stockfish.nnue", "ThirdParty/Stockfish/COPYING.txt"]
visible_files = source_files + resource_files + ["App/Info.plist", "Bridge/StockfishBridge.h"]
...
# Stockfish terminates the process when an NNUE file is present but invalid.
# Fail at project generation instead of allowing a build that exits shortly
# after the first search starts on a real device.
expected_networks = {
  "App/Resources/stockfish.nnue" => "c288c895ea924429ea9092e3f36b2b3c1f00f2a3a4c759ff7e57e79e3b43e4a7",
  "ThirdParty/Stockfish/src/nn-c288c895ea92.nnue" => "c288c895ea924429ea9092e3f36b2b3c1f00f2a3a4c759ff7e57e79e3b43e4a7",
  "ThirdParty/Stockfish/src/nn-37f18f62d772.nnue" => "37f18f62d772f3107e1d6aaca3898c130c3c86f2ab63e6555fbbca20635a899d"
}
invalid_networks = expected_networks.each_with_object([]) do |(path, expected), invalid|
  full_path = File.join(ROOT, path)
  invalid << path unless File.exist?(full_path) && Digest::SHA256.file(full_path).hexdigest == expected
end
abort "Invalid Stockfish 18 NNUE files: #{invalid_networks.join(', ')}" unless invalid_networks.empty?
...
group.call(main_group_id, PROJECT_NAME, ["#{app_group_id} /* App */", "#{bridge_group_id} /* Bridge */", "#{engine_group_id} /* Stockfish */", "#{products_group_id} /* Products */"])
...
group.call(engine_group_id, "Stockfish", engine_children.map { |id| "#{id} /* #{File.basename(file_refs.key(id))} */" })
...
target_common = {
  ...
  "HEADER_SEARCH_PATHS" => '("$(inherited)", "$(PROJECT_DIR)/ThirdParty/Stockfish/src")',
  ...
  "PRODUCT_BUNDLE_IDENTIFIER" => "com.yisi.chesscoach", "PRODUCT_NAME" => '"$(TARGET_NAME)"',
  "SWIFT_OBJC_BRIDGING_HEADER" => "Bridge/StockfishBridge.h", "SWIFT_VERSION" => "5.0",
