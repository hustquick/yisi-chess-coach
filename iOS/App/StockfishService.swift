...
enum StockfishError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .missingNetwork: "应用内缺少 stockfish.nnue 神经网络"
        case .engine(let message): "Stockfish：\(message)"
        case .malformedResponse: "Stockfish 返回了无法解析的数据"
        }
    }
}
...
final class StockfishService: @unchecked Sendable {
    static let shared = StockfishService()
    ...
    private let queue = DispatchQueue(label: "com.yisi.stockfish", qos: .utility)
    ...
    func apply(move: String, to fen: String) async throws -> String {
        try await onEngineQueue {
            ...
            if response.hasPrefix("error:") { throw StockfishError.engine(String(response.dropFirst(6))) }
            guard response.contains("/") else { throw StockfishError.malformedResponse }
            return response
        }
    }
    ...
    private func ensureInitialized() throws {
        ...
        guard let network = Bundle.main.path(forResource: "stockfish", ofType: "nnue") else {
            throw StockfishError.missingNetwork
        }
        let response: String = network.withCString { pointer in
            // Two search threads and a 64 MB hash leave ample headroom for
            // SwiftUI and the NNUE network on physical iPhones. Higher values
            // provided little benefit at coach depths but increased the risk
            // of memory-pressure termination after a move.
        ...
        if response.hasPrefix("error:") { throw StockfishError.engine(String(response.dropFirst(6))) }
        guard response == "ready" else { throw StockfishError.malformedResponse }
        initialized = true
    }
