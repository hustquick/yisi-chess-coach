...
本项目据此采用“棋盘优先”的信息层级：竖向或窄屏时棋盘优先、极简工具在上、折叠面板在下；横向或宽屏时棋盘与极简工具在左、折叠面板在右。极简工具统一为 `↶`、`↷`、`优`、`⇅`、`↻`；双人对弈、人机对战、摆盘工具、电脑等级和分析参数合并到侧栏的“对弈与分析设置”折叠模块，摆盘不会在棋盘附近新增面板；棋谱与存档排在辅助模块末端且默认折叠。折叠不会清除当前局面或配置，用户可随时恢复这些功能。
...
- 教练分析、局势图、棋谱和设置模块可折叠；对弈模式与分析参数统一收进“对弈与分析设置”。
