...
public final class MainActivity extends Activity implements ChessBoardView.Listener {
    ...
    private ChessBoardView board;private EvaluationChartView chart;private TextView status,turn,candidates,moves;private Button modeButton,sideButton,levelButton,undoButton,redoButton;private String fen=START,gameStatus="ongoing";private final List<String> history=new ArrayList<>(),fens=new ArrayList<>(),redoHistory=new ArrayList<>(),redoFens=new ArrayList<>();private int depth=14,eloIndex=4;private boolean flipped,computerMode,humanWhite=true,showBest=true;
    ...
    private String formatAnalysis(String json)throws Exception{
        ...
        for(int i=0;i<lines.length();i++){
            ...
            out.append(i+1).append("   ").append(pv.split(" ")[0]).append("   ")
                    .append(whiteScoreText(line.getString("score"))).append("   d").append(line.getInt("depth")).append('\n');
        }
        if(lines.length()>0&&chart!=null){String[] score=lines.getJSONObject(0).getString("score").trim().split("\\s+");if(score.length>1&&"cp".equals(score[0]))try{double value=Integer.parseInt(score[1])/100.0;if(fen.split(" ")[1].equals("b"))value=-value;final double white=value;main.post(()->chart.addValue(white));}catch(NumberFormatException ignored){}}
        return out.toString();
    }
    private String whiteScoreText(String score){String[] parts=score.trim().split("\\s+");if(parts.length<2)return score;try{int value=Integer.parseInt(parts[1]);if(fen.split(" ")[1].equals("b"))value=-value;if("mate".equals(parts[0]))return value>=0?"白方将杀":"黑方将杀";return String.format(java.util.Locale.US,"%+.2f",value/100.0);}catch(NumberFormatException e){return score;}}
    ...
    private void update(){turn.setText(!"ongoing".equals(gameStatus)?(gameStatus.equals("checkmate")?"对局结束":"和棋"):(fen.split(" ")[1].equals("w")?"白方走棋":"黑方走棋"));if(undoButton!=null){undoButton.setEnabled(!history.isEmpty());undoButton.setAlpha(history.isEmpty()?.35f:1f);}if(redoButton!=null){redoButton.setEnabled(!redoHistory.isEmpty());redoButton.setAlpha(redoHistory.isEmpty()?.35f:1f);}StringBuilder b=new StringBuilder();for(int i=0;i<history.size();i++){if(i%2==0)b.append(i/2+1).append(". ");b.append(history.get(i)).append(i%2==0?"   ":"\n");}moves.setText(b.length()==0?"尚未行棋":b);}
    ...
    private void undo(){if(history.isEmpty())return;generation.incrementAndGet();StockfishNative.stop();redoHistory.add(0,history.remove(history.size()-1));redoFens.add(0,fens.remove(fens.size()-1));fen=fens.get(fens.size()-1);gameStatus="ongoing";board.setFen(fen);board.setLastMove(history.isEmpty()?null:history.get(history.size()-1));update();analyze();}private void redo(){if(redoHistory.isEmpty())return;generation.incrementAndGet();StockfishNative.stop();String move=redoHistory.remove(0);fen=redoFens.remove(0);history.add(move);fens.add(fen);gameStatus="ongoing";board.setFen(fen);board.setLastMove(move);update();analyze();}private void reset(){generation.incrementAndGet();StockfishNative.stop();fen=START;gameStatus="ongoing";history.clear();fens.clear();redoHistory.clear();redoFens.clear();fens.add(fen);board.setFen(fen);board.setLastMove(null);update();if(computerMode&&!isHumanTurn())playComputerMove();else analyze();}
